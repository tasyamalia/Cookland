...
class MealFavAdapter(
    var data : List<FavoriteMeal>,
    private val listener: MealListener
) : RecyclerView.Adapter<MealFavAdapter.MealViewHolder>() {
    ...
    fun swapData(data: List<FavoriteMeal>) {
        this.data = data
        notifyDataSetChanged()
    }
    fun updateSelected(item: FavoriteMeal) {
        data.map {
    ...
    inner class MealViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(item: FavoriteMeal, listener: MealListener) = with(itemView) {
    ...
    interface MealListener {
        fun onClick(item: FavoriteMeal)
        fun onClickFavorite(item: FavoriteMeal)
    }
}